...
.book-card img {
    width: 100%;
    height: 160px; /* More compact image */
    object-fit: cover;
    border-radius: 6px;
...
""") as demo:
    ...
    favorite_book_id.render()
    favorite_action.render()

    # Connect favorites action to Python function
    def handle_favorite_change(book_id, action, favorites_df, favorites_display, favorites_count):
        if book_id and action:
            return trigger_favorite_action(book_id, action, favorites_df, favorites_display, favorites_count)
