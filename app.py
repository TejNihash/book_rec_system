...
""") as demo:
    ...
    # Favorites functions
    # Favorites trigger - UPDATED
    def handle_add_favorite(book_id):
        """Handle adding a favorite and return proper outputs"""
        favorites_df, html, load_more_visible, fav_count = add_to_favorites_simple(book_id)
        return favorites_df, html, load_more_visible, fav_count
    ...
    add_favorite_btn.click(
        ...
        outputs=[favorites_state, favorites_container, favorites_load_more_btn, favorite_book_id]  # Use favorite_book_id as proxy for count
    ).then(
        lambda count: update_favorites_count(count),
        inputs=[favorite_book_id],  # This now receives the count from previous function
    ...
    function addToFavorites(bookId) {
        ...
        // Find the hidden favorite components using more reliable selectors
        const favoriteIdInput = document.querySelector('input[aria-label="Favorite Book ID"]') || 
                               document.querySelector('input[value=""]');
        const addFavoriteBtn = Array.from(document.querySelectorAll('button')).find(btn => 
            btn.textContent.includes('Add Favorite') || 
            btn.innerHTML.includes('Add Favorite')
        );
        ...
        if (favoriteIdInput && addFavoriteBtn) {
            ...
            setTimeout(() => {
                ...
                console.log('✅ Favorite button clicked for book ID:', bookId);
            }, 200);
