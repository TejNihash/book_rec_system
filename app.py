...
def build_books_grid_html(books_df, is_favorites_section=False):
    if books_df.empty:
        if is_favorites_section:
            return "<div style='text-align: center; padding: 40px; color: #888; font-size: 16px;'>No favorite books yet. Click the favorite buttons to add some!</div>"
...
def create_favorite_buttons():
    """Create favorite buttons for all books"""
    favorite_buttons = {}
    for _, book in df.iterrows():
        book_id = book['id']
        is_fav = is_book_favorited(book_id)
        btn_text = "💔 Remove from Favorites" if is_fav else "❤️ Add to Favorites"
        favorite_buttons[book_id] = gr.Button(btn_text, visible=False)
...
.favorite-btn { background:linear-gradient(135deg,#ed8936 0%,#dd6b20 100%); color:white; border:none; padding:10px 20px; border-radius:20px; font-weight:600; cursor:pointer; transition:all 0.3s ease; box-shadow:0 4px 12px rgba(237,137,54,0.3); margin:10px 5px; font-size:14px; width:200px; }
...
""") as demo:
    ...
    with gr.Column():
        ...
        favorites_container = gr.HTML(
            ...
            value="<div style='text-align: center; padding: 40px; color: #888; font-size: 16px;'>No favorite books yet. Use the favorite buttons to add some!</div>"
        )
        favorites_load_more_btn = gr.Button("📚 Load More Favorites", elem_classes="load-more-btn", visible=False)

    # ---------- Visible Favorite Buttons for Each Book ----------
    gr.Markdown("## ⭐ Quick Favorites")
    gr.Markdown("### Add books to favorites directly:")
    
    # Create a grid of favorite buttons
    with gr.Row():
        favorite_buttons_row1 = []
        for i, (_, book) in enumerate(df.head(6).iterrows()):  # First 6 books
            is_fav = is_book_favorited(book['id'])
            btn_text = "💔 Remove" if is_fav else "❤️ Add to Favorites"
            btn = gr.Button(btn_text, elem_classes="favorite-btn")
            favorite_buttons_row1.append(btn)
    
    with gr.Row():
        favorite_buttons_row2 = []
        for i, (_, book) in enumerate(df.iloc[6:12].iterrows()):  # Next 6 books
            is_fav = is_book_favorited(book['id'])
            btn_text = "💔 Remove" if is_fav else "❤️ Add to Favorites"
            btn = gr.Button(btn_text, elem_classes="favorite-btn")
            favorite_buttons_row2.append(btn)

    # ---------- Hidden Components for State ----------
    with gr.Row(visible=False):
        current_book_id = gr.Textbox()
        current_action = gr.Textbox()
        trigger_update = gr.Button("Update Favorites")
    ...
    def handle_favorite_click(book_index, row_num):
        """Handle favorite button clicks"""
        if row_num == 1:
            book = df.iloc[book_index]
        else:
            book = df.iloc[6 + book_index]
        ...
        if success:
            ...
            # Update button text
            new_btn_text = "💔 Remove" if not is_fav else "❤️ Add to Favorites"
            
            # Create feedback message
            feedback_html = f"""
            <div class="feedback-toast" style="background: {'#48bb78' if not is_fav else '#f56565'}">
                {message}
            </div>
            """
            
            return favorites_df, html, load_more_visible, header, new_btn_text, feedback_html
        else:
            return favorites_state.value, favorites_container.value, favorites_load_more_btn.value, favorites_header.value, gr.update(), f"<div class='feedback-toast' style='background: #f56565'>{message}</div>"
    ...
    # Connect favorite buttons - Row 1
    for i, btn in enumerate(favorite_buttons_row1):
        btn.click(
            lambda x=i: handle_favorite_click(x, 1),
            outputs=[
                favorites_state, 
                favorites_container, 
                favorites_load_more_btn, 
                favorites_header,
                btn,
                feedback
            ]
    ...
    # Connect favorite buttons - Row 2
    for i, btn in enumerate(favorite_buttons_row2):
        btn.click(
            lambda x=i: handle_favorite_click(x, 2),
            outputs=[
                favorites_state, 
                favorites_container, 
                favorites_load_more_btn, 
    ...
    <script>
    // Simple popup for book details (optional)
    document.addEventListener('click', function(e) {
        ...
        content.innerHTML = `
            <div style="display: flex; gap: 20px; margin-bottom: 20px;">
                <img src="${img}" style="width: 150px; height: auto; border-radius: 8px;">
                <div>
                    <h2>${title}</h2>
