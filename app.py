...
def add_to_favorites(book_id, favorites_df):
    ...
    return new_favorites, display_favorites, gr.update(value=html), gr.update(visible=len(new_favorites) > BOOKS_PER_LOAD)
...
""") as demo:
    ...
    # === HIDDEN FAVORITES TRIGGER ===
    with gr.Row(visible=False):
        add_favorite_trigger = gr.Button("Add Favorite Trigger")
        favorite_book_id_input = gr.Textbox(value="", label="Book ID")
    ...
    # Favorites functions
    def add_favorite(book_id, favorites_df, favorites_display):
        if book_id:
            return add_to_favorites(book_id, favorites_df)
        return favorites_df, favorites_display, favorites_container, favorites_load_more_btn
    ...
    # Favorites trigger
    add_favorite_trigger.click(
        add_favorite,
        [favorite_book_id_input, favorites_state, favorites_display_state],
        [favorites_state, favorites_display_state, favorites_container, favorites_load_more_btn]
    )
    ...
    function addToFavorites(bookId) {
        ...
        // Click the hidden trigger button
        const triggerBtn = document.querySelector('button:contains("Add Favorite Trigger")');
        if (triggerBtn) {
            triggerBtn.click();
        }
        ...
        feedback.textContent = '❤️ Added to favorites!';
    ...
    document.addEventListener('click', function(e) {
        ...
        content.innerHTML = `
            ...
            <div class="favorite-action-section">
                <button class="favorite-btn" onclick="addToFavorites('${currentBookId}')">
                    🤍 Add to Favorites
                </button>
            </div>
        `;
