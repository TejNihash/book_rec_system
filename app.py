...
""") as demo:
    ...
    (function(){
        ...
        function setHiddenFavoriteIdAndTrigger(bookId){
            // Use the hidden textbox created by Gradio with elem_id="fav-id"
            const wrapper = (typeof gradioApp === 'function') ? gradioApp().getElementById('fav-id') : document.getElementById('fav-id');
            if(!wrapper){
                console.warn('fav-id wrapper not found');
                return;
            }
            // find the actual input/textarea inside that wrapper
            const input = wrapper.querySelector('input, textarea');
            if(!input){
                console.warn('fav-id input not found');
                return;
            }
            input.value = bookId;
            // dispatch events so Gradio picks up the change
