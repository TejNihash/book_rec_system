...
def get_similar_books(fav_ids, top_k=100):
    ...
    df["similarity"] = sims
    recs = df[~df["id"].isin(fav_ids)].sort_values("similarity", ascending=False).head(top_k)
    return recs
    ...
    with gr.Column(elem_classes="main-content"):
        ...
        with gr.Column(elem_classes="search-section"):
            ...
            with gr.Row(elem_classes="search-row"):
                search_input = gr.Textbox(placeholder="Search by title, author, genre, or description...", show_label=False, elem_classes="search-input")
        ...
        with gr.Column(elem_classes="scroll-section"):
            ...
            # Combined load more (random/search)
...
function updateFavoritesSidebar(){
  ...
  if(favorites.size === 0){ sidebarList.innerHTML = "<p>No favorites yet.</p>"; return; }
  let html = "";
  favorites.forEach((book,id)=>{
    html += `<div class="sidebar-book" data-id="${id}">
      ...
      <div style="flex:1;font-size:12px;color:#fff;"><strong>${escapeHtml(book.title)}</strong><br><span style="color:#aaa;">${escapeHtml(book.authors)}</span></div>
      <button class="remove-fav-btn" title="Remove from Favorites" style="background:none;border:none;color:#888;cursor:pointer;font-size:14px;">🗑️</button>
    </div>`;
  });
...
window.syncFavoritesToPython = function() {
    ...
    syncFavsBtn.dataset.favIds = JSON.stringify(fav_ids);
    syncFavsBtn.click();
}
document.getElementById('recs-refresh-btn')?.addEventListener("click", ()=>{ window.syncFavoritesToPython(); });
document.addEventListener('click', e=>{
  if(e.target.closest('.remove-fav-btn')){
    const parent = e.target.closest('.sidebar-book'); if(!parent) return;
    const id = parent.dataset.id; favorites.delete(id); updateFavoritesSidebar();
    const cardBtn = document.querySelector(`.book-card[data-id="${id}"] .fav-btn`);
    if(cardBtn) cardBtn.classList.remove('fav-active'); return;
  }
  const favBtn = e.target.closest('.fav-btn');
  if(favBtn){
    e.stopPropagation();
    const card = favBtn.closest('.book-card');
    const bookId = card.dataset.id; const title = card.dataset.title; const authors = card.dataset.authors; const img = card.dataset.img;
    if(favorites.has(bookId)){ favorites.delete(bookId); favBtn.classList.remove('fav-active'); } else { favorites.set(bookId,{title,authors,img}); favBtn.classList.add('fav-active'); }
    updateFavoritesSidebar(); syncFavoritesToPython(); return;
  }
  const card = e.target.closest('.book-card'); if(!card) return;
  const title = card.dataset.title; const authors = card.dataset.authors; const genres = card.dataset.genres; const desc = card.dataset.desc; const img = card.dataset.img;
  document.getElementById('detail-content').innerHTML = `<div style="display:flex;gap:16px;align-items:flex-start;color:#fff;"><img src="${img}" style="width:200px;height:auto;border-radius:6px;object-fit:cover;"><div style="max-width:260px;"><h2 style="margin:0 0 8px 0;color:#fff;">${escapeHtml(title)}</h2><p style="margin:0 0 4px 0;color:#fff;"><strong>Author(s):</strong> ${escapeHtml(authors)}</p><p style="margin:0 0 6px 0;color:#fff;"><strong>Genres:</strong> ${escapeHtml(genres)}</p><div><strong>Description:</strong></div><div class="desc-scroll">${escapeHtml(desc)}</div></div></div>`;
  const rect = card.getBoundingClientRect(); let left = rect.right + 10; let top = rect.top;
  ...
  box.style.left = `${Math.max(left, 10)}px`; box.style.top = `${Math.max(top, 10)}px`;
  overlay.style.display='block'; box.scrollIntoView({ behavior: "smooth", block: "nearest" });
});
