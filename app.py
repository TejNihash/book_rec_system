...
""") as demo:
    ...
    # ---------- Hidden Button to receive JS event ----------
    ...
    def add_to_favorites(book_id, fav_list):
        ...
        # Return updated favorites HTML
        fav_books = df[df['id'].isin(fav_list)]
        html = build_books_grid_html(fav_books)
    ...
    random_load_more_btn.click(load_more_random,
        [gr.State(df.sample(frac=1).reset_index(drop=True)), gr.State(pd.DataFrame()), gr.State(0)],
        [random_books_container, random_load_more_btn, gr.State(0)]
    )
    shuffle_btn.click(shuffle_random_books,
        [gr.State(df.sample(frac=1).reset_index(drop=True)), gr.State(pd.DataFrame())],
        [random_books_container, gr.State(pd.DataFrame()), gr.State(0)]
    )

    popular_load_more_btn.click(load_more_popular,
        [gr.State(df.copy()), gr.State(pd.DataFrame()), gr.State(0)],
        [popular_books_container, popular_load_more_btn, gr.State(0)]
    )

    # Hidden button click connects JS to Python
    hidden_btn.click(add_to_favorites, [gr.State("hidden-add-fav").value, favorites_state], [favorites_state, favorites_container])
