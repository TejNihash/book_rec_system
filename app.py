...
""") as demo:
    ...
    # Create book cards HTML - FIXED: Better JavaScript approach
    def create_book_cards_html(books_df, section_name):
        ...
        for _, book in books_df.iterrows():
            ...
            card_html = f"""
            <div class="book-card" data-book-id="{book['id']}" data-section="{section_name}">
        ...
        return f'<div class="book-grid" id="{section_name}-books">{"".join(cards_html)}</div>'
    
    # Hidden components for book selection - FIXED: Use proper Gradio components
    selected_book_id = gr.Textbox(visible=False, label="selected_book_id")
    selected_section = gr.Textbox(visible=False, label="selected_section")
    
    # JavaScript for book selection - FIXED: Better element selection
    demo.load(
        None,
        None,
        None,
        js="""
        function initializeBookClicks() {
            // Use event delegation for better reliability
            document.addEventListener('click', function(e) {
                const bookCard = e.target.closest('.book-card');
                if (bookCard) {
                    const bookId = bookCard.getAttribute('data-book-id');
                    const sectionName = bookCard.getAttribute('data-section');
                    
                    if (bookId) {
                        // Find the hidden inputs by their label
                        const bookIdInput = document.querySelector('input[label="selected_book_id"]');
                        const sectionInput = document.querySelector('input[label="selected_section"]');
                        
                        if (bookIdInput && sectionInput) {
        ...
        setTimeout(initializeBookClicks, 1000);
        """
    )
    ...
    def handle_refresh():
        ...
        return update_displays(random_books, current_popular_books.value)[0], random_books
    ...
    refresh_btn.click(
        handle_refresh,
        [],
        [random_display, current_random_books]
    )
